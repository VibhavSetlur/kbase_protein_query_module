...
class TestSimilarityIndex(unittest.TestCase):
    def setUp(self):
        ...
        self.family_id = 'FAMX'
        self.index.create_family_index(self.family_id, self.embeddings, self.protein_ids)
    ...
    def test_search_family(self):
        ...
        sims, ids = self.index.search_family(self.family_id, query, top_k=5)
    ...
    def test_cache_eviction(self):
        ...
        for i in range(5):
            fam = f'FAM{i}'
            self.index.create_family_index(fam, self.embeddings, self.protein_ids)
            self.index.search_family(fam, np.random.randn(8).astype(np.float32), top_k=1)
