...
class DynamicNetworkBuilder:
    ...
    # --- Compatibility convenience API expected by some tests ---
    def build_network(self, similarity_matrix: np.ndarray, protein_ids: List[str],
                      network_method: str = 'mutual_knn', k_neighbors: int = 8,
                      similarity_threshold: float = 0.5) -> Dict[str, Any]:
        """Build a network from a similarity matrix and return a portable dict.

        The returned dict contains 'nodes', 'edges', and 'network_metrics'.
        """
        # Update parameters temporarily
        original_knn = self.k_neighbors
        original_thr = self.similarity_threshold
        try:
            self.k_neighbors = k_neighbors
            self.similarity_threshold = similarity_threshold
            # Construct embeddings by projecting via similarity_matrix using first PC-like axes
            # For tests, we only need a graph structure; we can convert to cosine space by identity
            # Build according to chosen method
            if network_method == 'threshold':
                G = self.build_threshold_network(similarity_matrix, protein_ids)
            elif network_method == 'knn':
                # Use hybrid as a k-NN approximation
                G = self.build_hybrid_network(similarity_matrix, protein_ids)
            else:
                G = self.build_mutual_knn_network(similarity_matrix, protein_ids)
            props = self.analyze_network_properties(G)
            # Convert to simple dict
            nodes = list(G.nodes())
            edges = [{'source': u, 'target': v, 'weight': float(G[u][v].get('weight', 0.0))}
                     for u, v in G.edges()]
            return {
                'nodes': nodes,
                'edges': edges,
                'network_metrics': {
                    'num_nodes': props.get('num_nodes', len(nodes)),
                    'num_edges': props.get('num_edges', len(edges)),
                    'density': float(props.get('density', 0.0)),
                    'clustering_coefficient': float(props.get('average_clustering', 0.0))
                }
            }
        finally:
            self.k_neighbors = original_knn
            self.similarity_threshold = original_thr

    def detect_communities(self, network_dict: Dict[str, Any]) -> Dict[str, Any]:
        """Detect communities in a network dict produced by build_network."""
        G = nx.Graph()
        G.add_nodes_from(network_dict.get('nodes', []))
        for e in network_dict.get('edges', []):
            G.add_edge(e['source'], e['target'], weight=e.get('weight', 0.0))
        try:
            import networkx.algorithms.community as nx_comm
            comms = list(nx_comm.greedy_modularity_communities(G))
            modularity = nx_comm.modularity(G, comms) if len(comms) > 1 else 0.0
            communities = [list(c) for c in comms]
        except Exception:
            communities, modularity = [], 0.0
        return {'communities': communities, 'modularity': float(modularity)}

    def calculate_centrality(self, network_dict: Dict[str, Any]) -> Dict[str, Dict[str, float]]:
        """Compute centrality measures for a network dict produced by build_network."""
        G = nx.Graph()
        G.add_nodes_from(network_dict.get('nodes', []))
        for e in network_dict.get('edges', []):
            G.add_edge(e['source'], e['target'], weight=e.get('weight', 0.0))
        ...
        return {
            'degree_centrality': {k: float(v) for k, v in degree.items()},
            'betweenness_centrality': {k: float(v) for k, v in betweenness.items()},
            'closeness_centrality': {k: float(v) for k, v in closeness.items()},
            'eigenvector_centrality': {k: float(v) for k, v in eigenvector.items()}
        }

    def export_network(self, network_dict: Dict[str, Any], output_path: str, format: str = 'json') -> None:
        """Export a network dict to JSON, CSV or GML for tests."""
        import json, csv
