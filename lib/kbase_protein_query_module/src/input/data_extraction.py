...
from ..util.storage import ProteinExistenceChecker
...
class DataExtractionStage(BaseStage):
    ...
    def validate_input(self, input_data: Dict[str, Any]) -> bool:
        ...
        if 'validated_input' not in input_data and 'validated_data' not in input_data:
            logger.error("Missing validated_input")
            return False
        
        validated_input = input_data.get('validated_input')
        # Accept either dataclass InputData or dict alias 'validated_data'
        if validated_input is not None and hasattr(validated_input, 'input_type'):
            return True
        if isinstance(input_data.get('validated_data'), dict):
            return True
    ...
    def run(self, input_data: Dict[str, Any], workspace_client=None) -> StageResult:
        ...
        try:
            validated_input = input_data.get('validated_input')
            # Support dict alias pathway or top-level validated_data only
            if (validated_input is None or not hasattr(validated_input, 'input_type')) and isinstance(input_data.get('validated_data'), dict):
                vdict = input_data['validated_data']
                class _VD: pass
                shim = _VD()
                shim.input_type = vdict.get('input_type', 'generic')
                shim.input_source = 'generic'
                shim.protein_records = [input_parser.ProteinRecord(protein_id=d.get('protein_id', ''), sequence=d.get('sequence', ''), source='generic') for d in vdict.get('protein_records', [])]
                validated_input = shim
        ...
        for i in range(0, len(validated_input.protein_records), self.batch_size):
            ...
            for record in batch:
                try:
                    ...
                    uniprot_data = self._fetch_uniprot_data(record.protein_id)
                    if uniprot_data:
                        protein_records.append(uniprot_data)
                        extraction_stats['successfully_extracted'] += 1
                    else:
                        extraction_stats['failed_extractions'] += 1
                        extraction_stats['errors'].append(f"No data found for {record.protein_id}")
                        
                except Exception as e:
                    extraction_stats['failed_extractions'] += 1
                    extraction_stats['errors'].append(f"Failed to extract {record.protein_id}: {str(e)}")
