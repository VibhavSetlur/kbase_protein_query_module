...
class InputValidationStage(BaseStage):
    ...
    def run(self, input_data: Dict[str, Any], workspace_client=None) -> StageResult:
        ...
        try:
            ...
            if input_type in ('uniprot_identifiers', 'uniprot_ids'):
                validated_data = self._validate_uniprot_identifiers(input_data_raw, validation_summary)
            elif input_type in ('fasta', 'fasta_sequence'):
                # Accept raw FASTA string or file path
                if isinstance(input_data_raw, str) and input_data_raw.strip().startswith('>'):
            ...
            elif input_type in ('fasta_file', 'fasta_path'):
                validated_data = self._validate_fasta_file(input_data_raw, validation_summary)
            ...
            # Also provide a flattened alias key expected by some tests
            return StageResult(
                success=True,
                output_data={
                    ...
                    'validated_data': {
                        'protein_records': [
                            {'protein_id': r.protein_id, 'sequence': r.sequence} for r in getattr(validated_data, 'protein_records', [])
                        ],
                        'input_type': getattr(validated_data, 'input_type', input_type)
                    }
                },
                metadata={
    ...
    def _validate_uniprot_identifiers(self, identifiers: List[str], summary: Dict[str, Any]) -> InputData:
        ...
        return InputData(
            protein_records=[input_parser.ProteinRecord(protein_id=pid, source="uniprot", sequence="") for pid in valid_identifiers],
