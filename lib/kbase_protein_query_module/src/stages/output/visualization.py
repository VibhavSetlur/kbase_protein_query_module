    ...
    def run(self, input_data, workspace_client=None):
        ...
        try:
            ...
            for i, embedding in enumerate(embeddings):
                ...
                try:
                    # Get network analysis results for this protein
                    network_result = network_results.get(protein_id, {})
                    if network_result.get('status') != 'success':
                        logger.warning(f"No successful network results for {protein_id}, skipping visualization")
                        continue

                    # Determine the set of proteins to visualize
                    # Prefer top_similar_proteins from network analysis results
                    top_similar = network_result.get('top_similar_proteins') or []
                    similar_ids = [p.get('protein_id') for p in top_similar if p and p.get('protein_id')]
                    # Ensure the query protein is included
                    candidate_ids = [pid for pid in similar_ids if pid is not None]
                    if protein_id not in candidate_ids:
                        candidate_ids.append(protein_id)

                    # Intersect with available protein_ids to keep only those with embeddings
                    pid_to_index = {pid: idx for idx, pid in enumerate(protein_ids)}
                    network_proteins = [pid for pid in candidate_ids if pid in pid_to_index]
                    if len(network_proteins) < 2:
                        logger.warning(f"Insufficient proteins with embeddings for {protein_id}, skipping visualization")
                        continue

                    # Build embeddings array for the selected proteins
                    indices = [pid_to_index[pid] for pid in network_proteins]
                    network_embeddings = embeddings[indices]

                    # Build metadata for visualization
                    try:
                        # Create metadata DataFrame for network proteins
                        if metadata_df.empty:
                            ...
                            # Use provided metadata, filtering for network proteins (fall back to minimal rows if some missing)
                            present = metadata_df.index.intersection(network_proteins)
                            viz_metadata = metadata_df.loc[present].copy()
                            missing = [pid for pid in network_proteins if pid not in present]
                            if missing:
                                minimal = pd.DataFrame(index=missing)
                                minimal['Protein names'] = [f'Protein {pid}' for pid in missing]
                                minimal['Organism'] = ['Unknown'] * len(missing)
                                minimal['Function [CC]'] = ['No metadata available'] * len(missing)
                                viz_metadata = pd.concat([viz_metadata, minimal], axis=0)
