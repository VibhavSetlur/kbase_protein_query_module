...
class ProteinFamilyAssigner:
    ...
    def predict_family(self, embedding: Union[np.ndarray, list, Any]) -> Tuple[str, float]:
        """
    ...
    def assign_families_batch(self, protein_embeddings: Dict[str, np.ndarray]) -> Dict[str, Dict[str, Any]]:
        """Assign multiple embeddings; returns mapping protein_id -> assignment result."""
        results: Dict[str, Dict[str, Any]] = {}
        for protein_id, emb in protein_embeddings.items():
            try:
                results[protein_id] = self.assign_family(emb)
            except Exception as e:
                results[protein_id] = {
                    'family_id': None,
                    'confidence': 0.0,
    ...
    def assign_family_with_threshold(self, embedding: np.ndarray, confidence_threshold: float = 0.5) -> Dict[str, Any]:
        """Assign family only if confidence >= threshold; else return None assignment."""
        result = self.assign_family(embedding)
    ...
    def assign_family_with_metadata(self, embedding: np.ndarray, protein_metadata: Dict[str, Any]) -> Dict[str, Any]:
        """Assign family and include provided metadata in the result."""
        result = self.assign_family(embedding)
        result['metadata'] = protein_metadata
        return result
    ...
    def search_similar_proteins(self, embedding: np.ndarray, family_id: str, top_k: int = 10) -> List[Dict]:
        ...
        """
        from .protein_storage import ProteinStorage
        try:
            storage = ProteinStorage()
            results = storage.search_within_family(
                family_id=family_id,
            ...
            for r in results:
                similar.append({
                    'protein_id': r.get('protein_id') or r.get('id') or r.get('pid'),
